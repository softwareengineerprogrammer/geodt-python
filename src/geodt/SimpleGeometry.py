...
def writeStlObject(points, simplices, fd):
    for simplex in simplices:
        ...
        for iPt in simplex:
            # print iPt,simplex
            fd.write(f'vertex {points[iPt][0]:g} {points[iPt][1]:g} {points[iPt][2]:g}\n')
        fd.write('endloop\n')
        fd.write('endfacet\n')
...
def writeVtk(objectList, scalars, scalarNames, vtkFile, name='vtkObjects'):
    ...
    for pts, simps in objectList:
        for pt in pts:
            fd.write(f'{pt[0]:g} {pt[1]:g} {pt[2]:g}\n')
...
def mergeObj(obj1, obj2):
    ...
    if obj2 == emptyObject:
        return obj1
    return (np.vstack((obj1[0], obj2[0])), np.vstack((obj1[1], obj2[1] + len(obj1[0]))))
...
unitWedgePts = np.asarray([[-0.5, -0.5, -0.5], [0.5, -0.5, -0.5], [0.0, 0.5, -0.5], [-0.5, -0.5, 0.5], [0.5, -0.5, 0.5], [0.0, 0.5, 0.5]])

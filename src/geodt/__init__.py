...
import geodt.SimpleGeometry as sg
...
def exponential_trunc(nsam, bval=1.0, Mmax=5.0, Mwin=1.0, prob=0.1):
    """
...
def contact_trunc(nsam, weight=0.15, bd_nom=0.002, stddev=0.5 * 0.002, exp_B=0.5, exp_C=0.5 / np.pi, bd_min=0.0001, bd_max=3.0 * 0.002):
    """
...
def lognorm_trunc(nsam, logmu=0.0, logdev=1.0, loglo=-2.0, loghi=2.0):
    """
...
def norm_trunc(nsam, mu=0.0, dev=1.0, lo=-2.0, hi=2.0):
    """
    get random samples from a log normal distribution
    """
...
class Cauchy:
    ...
    def set_sigG_from_Principal(self, Sh, SH, SV, ShAzn, ShDip):
        ...
        sigP = np.asarray([[Sh, 0.0, 0.0], [0.0, SH, 0.0], [0.0, 0.0, SV]])
        # Rotate about z-axis
    ...
    def plot_Pc(self, phi, mcc, filename='Pc_stereoplot.png'):
        ...
        pylab.pcolormesh(
            dip_dir_radians + np.pi,
            dip_angle_deg,
            np.ma.masked_where(np.isnan(criticalDelPpG), criticalDelPpG),
            vmin=110.0 * MPa,
            vmax=150.0 * MPa,
            cmap='rainbow_r',
        )  # vmin=0.0*MPa, vmax=self.SH,cmap='rainbow_r')
        ax.grid(True)
...
class Reservoir:
    ...
    def __init__(self):
        ...
        self.fNum = np.asarray([10, 10, 10], dtype=int)  # count
        self.fDia = np.asarray([[300.0, 900.0], [300.0, 900.0], [300.0, 900.0]], dtype=float)  # m
        self.fStr = np.asarray([[79.0 * deg, 8.0 * deg], [0.0 * deg, 8.0 * deg], [180.0 * deg, 8.0 * deg]], dtype=float)  # m
        self.fDip = np.asarray([[60.0 * deg, 8.0 * deg], [90.0 * deg, 8.0 * deg], [0.0 * deg, 8.0 * deg]], dtype=float)  # m
...
class Surface:
    ...
    def __init__(self, x0=0.0, y0=0.0, z0=0.0, dia=1.0, stk=0.0 * deg, dip=90.0 * deg, ty='fracture', rock=Reservoir(), mcc=-1, phi=-1):
        # *** base parameters ***
        ...
        self.prop_alpha = norm_trunc(1, rock.prop_alpha[1], rock.prop_alpha[1], rock.prop_alpha[0], rock.prop_alpha[2])[
            0
        ]  # proppant compressibility modulus
        self.roughness = (
            rock.f_roughness if type(rock.f_roughness) is float else np.random.uniform(rock.f_roughness[0], rock.f_roughness[2], (1))[0]
        )  # open flow roughness
        ...
        self.bh = norm_trunc(1, rock.bh[1], rock.bh[1], rock.bh[0], rock.bh[2])[
            0
        ]  # !!! would be nice to replace this with a physics based estimate
        if phi < 0:
    ...
    def check_integrity(self, rock=Reservoir(), pres=0.0):
        ...
        if self.Pc > Pc0:
            print(
                'alert: critical fracture at Tau = {:.2e}, sn = {:.2e}, and Pp {:.2e} having phi = {:.2e} rad, mcc = {:.2e} Pa, Pc = {:.2e} Pa adjusted to phi = {:.2e} rad, mcc = {:.2e} Pa, Pc = {:.2e} Pa'.format(
    ...
    def make_critical(self, rock=Reservoir(), pres=0.0):
        ...
        # output message
        print(f'         hydrofrac critical cohesion calculated as {self.mcc:.2e} Pa to obtain Pc = {self.Pc:.2e} Pa')
        # error case
        if self.Pc <= self.sn:
    ...
    def check_integrity_old(self, rock=Reservoir(), pres=0.0):
        ...
        if self.Pc > Pc0:
            print(
                'alert: critical fracture at Tau = {:.2e}, sn = {:.2e}, and Pp {:.2e} having mcc = {:.2e} Pa, phi = {:.2e} rad, Pc = {:.2e} Pa adjusted to phi = {:.2e} rad, mcc = {:.2e} Pa, Pc = {:.2e} Pa'.format(
...
class Mesh:
    ...
    def hydromech(self, f_id, fix=False):  # , pp=-666.0):
        ...
        # proppant propped fracture
            ...
            # total dilated aperture
            bd = bd0p * np.exp(-self.faces[f_id].prop_alpha * (e_max - e_crit)) + bd0 * np.exp(-self.faces[f_id].u_alpha * e_max)
            # total hydraulic aperture
            bh = (qp + qs) ** (1.0 / 3.0)
            # note that fracture is not hydropropped
            self.faces[f_id].hydroprop = False
    ...
    def build_vtk(self, fname='default', vtype=[1, 1, 1, 1, 1, 1]):
        ...
        if vtype[1] and len(self.faces) > 6:
            ...
            for i in range(6, len(self.faces)):  # skip boundary node at np.inf
                ...
                # add geometry
                f_obj += [
                    HF(r=0.5 * self.faces[i].dia, x0=self.faces[i].c0, strikeRad=self.faces[i].str, dipRad=self.faces[i].dip, h=0.01 * r)
                ]
            # vtk file
                ...
                if self.faces[i].ci >= 0:
                    ...
                    q_obj += [
                        HF(
                            ...
                            x0=self.faces[i].c0,
                            strikeRad=self.faces[i].str,
                            dipRad=self.faces[i].dip,
                            h=0.02 * r,
                        )
                    ]
            # vtk file
        ...
        if vtype[3]:
            ...
            for i in range(1, self.nodes.num):  # skip boundary node at np.inf
                ...
                # add geometry
                n_obj += [
                    sg.cylObj(x0=self.nodes.all[i] + np.asarray([0.0, 0.0, -r]), x1=self.nodes.all[i] + np.asarray([0.0, 0.0, r]), r=r)
                ]
            # vtk file
    ...
    def build_pts(self, spacing=25.0, fname='test_gridx'):
        ...
        for i in range(0, self.pipes.num):
            ...
            elif int(self.pipes.typ[i]) in [
                typ('fracture'),
                typ('propped'),
                typ('choke'),
            ]:  # (int())Y[i][2] == 1: #fracture, effective cubic law
                # fracture info
        ...
        head += f'ORIGIN {o0[0]:f} {o0[1]:f} {o0[2]:f}\n'
        head += f'SPACING {ss[0]:f} {ss[1]:f} {ss[2]:f}\n'
        head += 'POINT_DATA %i\n' % (ns[0] * ns[1] * ns[2])
    ...
    def set_bcs(self, p_bound=0.0 * MPa, q_well=[], p_well=[]):
        ...
        for w in range(0, len(self.wells)):
            ...
            if not (ck):  # yes duplicate
                ...
                if q_well[w] is not None:
                    self.Q += [[i, -q_well[w]]]
                # pressure boundary conditions
                elif p_well[w] is not None:
    ...
    def x_well_all_faces(
        self, plot=True, sourceID=0, targetID=[], offset=[]
    ):  # , path_type=0, aperture=0.22, roughness=80.0): #[x0,y0,zo,len,azn,dip]
        ...
        if len(x_well) == 0:
            ...
            self.add_flowpath(c0, c1 + offset, leg, dia, lty, sourceID, Dh=dia, Dh_max=dia, frict=self.wells[sourceID].rgh)
            # return False
            return False
        # in case of intersections
            ...
            self.add_flowpath(c0, x_well[a[0]] + offset, rs[a[0]], dia, lty, sourceID, Dh=dia, Dh_max=dia, frict=self.wells[sourceID].rgh)
            # intersection points
            i = 0
    ...
    def x_frac_face(self, plot=True, sourceID=0, targetID=1):  # [x0,y0,z0,dia,azn,dip]
        ...
        if np.dot(vNor1, vNor2) < 0.999999:
            ...
            cInt = np.asarray([np.linalg.det([[vN1[1], vN2[1]], [d1, d2]]), np.linalg.det([[d1, d2], [vN1[0], vN2[0]]])]) / np.linalg.det(
                [[vN1[0], vN2[0]], [vN1[1], vN2[1]]]
            )
            ...
            for i in range(0, slot + 1):
                if (
                    (np.linalg.norm(xInt[slot - i] - c01) >= 1.01 * (dia1))
                    or (np.linalg.norm(xInt[slot - i] - c02) >= 1.01 * (dia2))
                    or (np.sum(np.isnan(xInt)) > 0)
                ):
    ...
    # ********************************************************************
    def gen_fixfrac(self, clear=True, c0=[0.0, 0.0, 0.0], dia=500.0, azn=80.0 * deg, dip=90.0 * deg):
    ...
    # fracture network creation
    # ********************************************************************
    def gen_natfracs(self, clear=True, f_num=40, f_dia=[200.0, 900.0], f_azn=[79.0 * deg, 8.0 * deg], f_dip=[90.0 * deg, 12.5 * deg]):
    ...
    def gen_joint_sets(self):
        ...
        for i in range(0, len(self.rock.fNum)):
            self.gen_natfracs(False, self.rock.fNum[i], self.rock.fDia[i], self.rock.fStr[i], self.rock.fDip[i])
    ...
    def gen_stimfracs(
        ...
        for _n in range(0, num):
            ...
            # add to model domain
            self.hydfs += [Surface(c0[0], c0[1], c0[2], leg, azn, dip, 'propped', self.rock, mcc=self.rock.hfmcc, phi=self.rock.hfphi)]
    ...
    def gen_wells(self, clear=True, wells=[]):
        ...
        # automatic placement
            ...
            dip = self.rock.w_dip
            vInj = np.asarray([math.sin(azn) * math.cos(-dip), math.cos(azn) * math.cos(-dip), math.sin(-dip)])
            vRht = np.asarray([math.sin(azn + pi / 2), math.cos(azn + pi / 2), 0.0])
            vNor = np.asarray([math.sin(azn) * math.sin(-dip), math.cos(azn) * math.sin(-dip), math.cos(-dip)])
            # offset center
            ...
            for i in range(0, seg):
                wells += [
                    Line(
                        i1s[i][0], i1s[i][1], i1s[i][2], leg, azn, dip, 'injector', self.rock.ra, self.rock.rb, self.rock.rc, self.rock.rgh
                    )
                ]
            ...
            for i in range(0, num):
                ...
                wells += [
                    Line(
                        p1s[i][0], p1s[i][1], p1s[i][2], pLen, azn, dip, 'producer', self.rock.ra, self.rock.rb, self.rock.rc, self.rock.rgh
                    )
                ]
            # add to model domain
            self.wells = wells
    ...
    # ************************************************************************
    def add_frac(
        self, typ='propped', c0=np.asarray([0.0, 0.0, 0.0]), dia=[1266.0, 107.0], azn=[65.0 * deg, 4.0 * deg], dip=[57.5 * deg, 3.75 * deg]
    ):
                ...
                for s in s_s:
                    if s >= 0:
                        ...
                        for t in range(0, len(self.faces)):
                            ...
                            if (
                                (s != t) and (t >= 0) and (ck_3 == 0) and s not in track[:, 0]
                            ):  # this pair is fresh, check for intersections
    ...
    def get_power(self, detail=False):
        ...
        for t in range(0, len(self.p_hm) - 1):
            ...
            Binary = (
                ms * nBs * np.max([0.0, hBis - hBo]) + ml * nBl * np.max([0.0, hBil - hBo])
            ) * effic  # kW, power produced from binary cycle
        ...
        if detail:
            ...
            print(f'Inject (5): T= {T5:.2f}; P= {P5:.2f}; h= {h5:.2f}, s= {s5:.4f}, x= {x5:.4f}, v= {v5:.6f}')
            print(f'Reserv (r,1): T= {Tr:.2f}; P= {Pr:.2f}; h= {hr:.2f}, s= {sr:.4f}, x= {xr:.4f}, v= {vr:.6f}')
            print(f'Produc (2): T= {T2:.2f}; P= {P2:.2f}; h= {h2:.2f}, s= {s2:.4f}, x= {x2:.4f}, v= {v2:.6f}')
            print(f'Turbi (2s): T= {T2s:.2f}; P= {P2s:.2f}; h= {h2s:.2f}, s= {s2s:.4f}, x= {x2s:.4f}, v= {v2s:.6f}')
            print(f'Brine (2l): T= {T2l:.2f}; P= {P2l:.2f}; h= {h2l:.2f}, s= {s2l:.4f}, x= {x2l:.4f}, v= {v2l:.6f}')
            print(f'Exhau (3s): T= {T3s:.2f}; P= {P3s:.2f}; h= {h3s:.2f}, s= {s3s:.4f}, x= {x3s:.4f}, v= {v3s:.6f}')
            print(f'Conde (4s): T= {T4s:.2f}; P= {P4s:.2f}; h= {h4s:.2f}, s= {s4s:.4f}, x= {x4s:.4f}, v= {v4s:.6f}')
            print('*** Binary Cycle Thermal State Values ***')
            print(f'Steam: Ti= {TBis:.2f}; Pi= {PBis:.2f}; hi= {hBis:.2f} -> To= {TBo:.2f}, Po= {PBo:.2f}, ho= {hBo:.2f}, n ={nBs:.3f}')
            print(f'Brine: Ti= {TBil:.2f}; Pi= {PBil:.2f}; hi= {hBil:.2f} -> To= {TBo:.2f}, Po= {PBo:.2f}, ho= {hBo:.2f}, n ={nBl:.3f}')
            print('*** Power Output Estimation ***')
            print('Turbine Flow Rate = %.2f kg/s' % (ms))
            print('Bypass Flow Rate = %.2f kg/s' % (ml))
            print(f'Well Flow Rate = {mt:.2f} kg/s = {Vt:.2f} L/min')
    ...
    def get_heat(
        ...
        xr = state.x  # steam quality
        vr = state.v  # m3/kg
        print(f'Reserv (r): T= {Tr:.2f}; P= {Pr:.2f}; h= {hr:.2f}, s= {sr:.4f}, x= {xr:.4f}, v= {vr:.6f}')
        ...
        ERor = (
            ...
            * 1.0
            * (
                (1 - np.log(Rir) / np.log(Rir / Ror)) * (Ror**2 - Rir**2) / 2
        ...
        for t in range(0, len(ts) - 1):  # !!! add self-estimation of dE0 for stabilization
                ...
                for p in range(0, Np):
                    ...
                    if (iters - 1) % E0_update_intervals == 0:
                        ...
                        if int(self.pipes.typ[p]) in [typ('injector'), typ('producer'), typ('pipe')]:  # pipe, radial heat flow
                            ...
                            Qt[t, p] = Lp[p] / (
                                1.0 / (2.0 * pi * Ris * H)
                                + np.log(R0[p] / Rir) / (2.0 * pi * ResKt * 10**-3)
                                + np.log(Rir / Ric) / (2.0 * pi * CemKt * 10**-3)
                            )  # kJ/K-s
                        elif int(self.pipes.typ[p]) in [
                            typ('boundary'),
                            typ('fracture'),
                            typ('propped'),
                            typ('darcy'),
                            typ('choke'),
                        ]:  # fracture, plate heat flow
            ...
            for i in range(0, Np):
                ...
                if int(self.pipes.typ[i]) in [typ('injector'), typ('producer'), typ('pipe')]:  # pipe, radial heat flow
                    ...
                    Qt[t + 1, i] = Lp[i] / (
                        1.0 / (2.0 * pi * Ris * H)
                        + np.log(R0[i] / Rir) / (2.0 * pi * ResKt * 10**-3)
                        + np.log(Rir / Ric) / (2.0 * pi * CemKt * 10**-3)
                    )  # kJ/K-s # Note converted Kt in W/m-K to kW/m-K
    ...
    def dyn_stim(
        ...
        # user status update
        print(f'+> dynamic stimulation with injection volume (Vinj) = {Vinj:.1e} m3 and rate (Qinj) = {Qinj:.3e} m3/s')
        ...
        while 1:
            ...
            for i in range(0, self.pipes.num):
                if int(self.pipes.typ[i]) in [
                    typ('boundary'),
                    typ('fracture'),
                    typ('propped'),
                    typ('choke'),
                ]:  # don't confuse well ID with face ID
            ...
            for i in range(0, i_div):
                ...
                if not (completed[i]):
                    ...
                    elif (self.wells[i_key[i]].hydrofrac is False) and (tip[i] > (self.rock.s3 + self.rock.hfmcc)):
                    ...
                    elif (nat_stim is False) or (((int(num_stim) + 1) % int(self.rock.stim_limit)) == 0):
                        dpi[i] += self.rock.dPi
    ...
    def stim_and_flow(
        ...
        # Solve stimulation
        self.dyn_stim(Vinj=Vstim, Qinj=Qstim, target=target, visuals=visuals, fname=(fname + '_stim'))

        # Solve production
        self.dyn_stim(Vinj=Vinj, Qinj=Qinj, target=target, visuals=visuals, fname=(fname + '_prod'))
    ...
    def detournay_visc(self, Q0=0.08):
        ...
        for t in time:
            ...
            for p in range(0, len(rho)):
                ...
                scaled_aper = ((0.6846 * 70.0**0.5) / 3.0 + (13.0 * rho[p] - 6.0) * (0.07098 * 4.0 * 5.0**0.5) / 9.0) * (
                    1.0 - rho[p]
                ) ** (2.0 / 3.0) + 0.09269 * ((1.0 - rho[p]) ** 0.5 * 8.0 / pi - np.arccos(rho[p]) * rho[p] * 8.0 / pi)
    ...
    def get_economics(
        ...
        if detail:
            print('\n*** economics module ***')
            print('   sales: ${:.0f} ({:.2f} kWh)'.format(P, NPsum * dt * 24.0 * 365.2425))
            print(f'   capital: ${C:.0f} ({drill_len:.2f} m drilled length)')
            print(f'   seismic: ${Q:.0f} ({Max_Quake:.2f} Mw max quake)')
            print('   NPV: $%.0f' % (NPV))
...
if __name__ == '__main__':  # main program
    ...
    for s in range(0, len(spacing)):
        ...
        for f in flows:
            # set boundary conditions (m3/s) (Pa); 10 kg/s ~ 0.01 m3/s for water
            geom.set_bcs(plot=False, p_bound=h_bh, q_inlet=[f], p_outlet=[dpps, dpps])
